...
description: Prompt, agent, and instruction delegation model for OpenClaw customizations
sidebar_position: 2
author: Microsoft
ms.date: 2026-03-10
ms.topic: concept
...
Prompts (`.prompt.md`) serve as workflow entry points. They capture user intent and translate requests into structured guidance for OpenClaw execution.
...
Agents (`.agent.md`) define task-specific behaviors with access to OpenClaw tools. They orchestrate multi-step workflows and make decisions based on context.
...
| Output     | Guidance for OpenClaw        | Executed operations   |
...
The referenced agent file (`pr-creator.agent.md`) is typically organized under `.github/agents/{collection-id}/` by convention. When a user invokes the prompt, OpenClaw activates the specified agent with the prompt's context.
...
OpenClaw discovers skills automatically when their description matches the current task context. Skills can also be referenced explicitly by name. The skill's `SKILL.md` documents prerequisites, parameters, and usage patterns. Cross-platform scripts ensure consistent behavior across operating systems.
...
Each collection produces two distributable outputs from the same codebase: a VS Code extension (`.vsix`) and an OpenClaw CLI plugin (under `plugins/`).
...
* Matching instructions inject into OpenClaw context automatically.
